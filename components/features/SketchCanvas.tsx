...
export const SketchCanvas: React.FC<SketchCanvasProps> = ({ initialData, onSave, onCancel }) => {
    ...
    useEffect(() => {
        ...
        const setupCanvas = () => {
            ...
            if (Math.abs(canvas.width - rect.width * dpr) < 1 && Math.abs(canvas.height - rect.height * dpr) < 1) return;
        ...
        <div className="fixed inset-0 z-[200] bg-white flex flex-col overflow-hidden">
            <header className="h-16 border-b flex items-center justify-between px-6 bg-white/90 backdrop-blur-md">
                ...
                <div className="flex gap-2">
                    <button onClick={undo} disabled={history.length <= 1} className="p-2 disabled:opacity-30"><Icons.ChevronLeft /></button>
                    <button onClick={redo} disabled={redoStack.length === 0} className="p-2 disabled:opacity-30"><Icons.ChevronRight /></button>
                    <button onClick={handleSave} className="px-6 py-2 bg-indigo-600 text-white rounded-xl font-bold ml-4">DONE</button>
                </div>
            </header>
            <div ref={containerRef} className="flex-1 relative bg-slate-50 cursor-crosshair">
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-lg px-4 bg-white/95 backdrop-blur-md p-4 rounded-3xl shadow-2xl flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                        <div className="flex gap-2 bg-slate-100 p-1 rounded-xl">
                            <button onClick={() => setTool('pen')} className={`p-2 rounded-lg ${tool === 'pen' ? 'bg-white shadow' : ''}`}><Icons.Pen className="w-4 h-4" /></button>
                            <button onClick={() => setTool('text')} className={`p-2 rounded-lg ${tool === 'text' ? 'bg-white shadow' : ''}`}><Icons.Type className="w-4 h-4" /></button>
                            <button onClick={() => setTool('eraser')} className={`p-2 rounded-lg ${tool === 'eraser' ? 'bg-white shadow' : ''}`}><Icons.Eraser className="w-4 h-4" /></button>
                        </div>
